...
pub const SEC_PARAM_8_2: usize = ((SECURITY_PARAMETER + 8 - 1) / 8) * 2;
...
impl Serialize for BitVector {
        ...
        S: Serializer,
    {
        ...
        let seq = serializer.serialize_newtype_struct("BitVector", bytes);
        seq
    }
}
...
impl<'de> Deserialize<'de> for BitVector {
    ...
    where
        D: Deserializer<'de>,
    {
        let t: [u8; SEC_PARAM_8] = Deserialize::deserialize(de)?;
        Ok(BitVector::from_bytes(&t))
    }
}
...
impl DoubleBitVector {
    ...
    pub fn from_bytes(bytes: &[u8; SEC_PARAM_8_2]) -> Self {
        let u64s = bytes
            .chunks_exact(8)
            .map(|chunk| u64::from_le_bytes(chunk.try_into().unwrap()));
        let mut out = [0u64; Self::SIZE];
        for (o, u) in out.iter_mut().zip(u64s) {
            *o = u;
        }
        Self(out)
    ...
    pub fn bytes(&self) -> [u8; SEC_PARAM_8_2] {
        let mut out = [0u8; SEC_PARAM_8_2];
        for (i, x_i) in self.0.iter().enumerate() {
            out[8 * i..8 * (i + 1)].copy_from_slice(&x_i.to_le_bytes())
        }
        out
...
impl Serialize for DoubleBitVector {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let bytes = &self.bytes();
        let seq = serializer.serialize_newtype_struct("DoubleBitVector", bytes);
        seq
    }
}

impl<'de> Deserialize<'de> for DoubleBitVector {
    fn deserialize<D>(de: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        let t: [u8; SEC_PARAM_8_2] = Deserialize::deserialize(de)?;
        Ok(DoubleBitVector::from_bytes(&t))
    }
}
